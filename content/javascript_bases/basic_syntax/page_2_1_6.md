...
##### reduce
mapメソッドは配列の各要素を変換しますが、`reduceメソッド`は配列全体を変換します。
reduce（縮小する）という意味の通り、要素を一つずつ取り出して行う処理を配列全体をまとめて行うことで処理を`縮小`しています。
mapやfilterとの違いは、引数として渡す関数の第1仮引数が異なります。
`reduceメソッド`の第1仮引数は「アキュムレータ（accumulator）」といい、最終的な戻り値となります。
下記のサンプルでは`sum` (アキュムレータ)にオブジェクトの価格を加算してその合計を求めています。
```js
var fruits = [{name: 'apple', price: 300}, {name: 'banana', price: 400}];
// 第1引数は要素そのもの、第2引数は添字、第3引数は配列そのものが設定されます。
var values = fruits.reduce(function(sum, fruits) {
  return sum += fruits.price;
});
