...
これは、間の設計書（クラス）をもとに人間（インスタンス）を作っているというイメージです。
...
この慣習を守ることで、小文字で書くインスタンス名と大文字で書くクラス名が一目で区別をつけることができます。
{{% /notice %}}
...
function classes_pattern_2() {
  ...
  human.name = 'Suzuki Taro'; // getterでYamada TaroからSuzuki Taroに改名
...
このように各インスタンスで共有で利用できるメソッドを**プロトタイプメソッド（インスタンスメソッド）**と呼ばれています。  
...
このようにオブジェクト自身のプロトタイプを追っていくことを**プロトタイプチェーン**と呼ばれています。
