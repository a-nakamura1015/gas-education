...
`index.html`を下記の通りに変更したのち、Web画面を更新してコンソールログ値が表示されるかを確認してみましょう。
...
実は`getSheetData()`が実行されていますが、その戻り値が返ってくる前に`console.log(result)`が実行されているのです。  
...
しかし、GAS関数の戻り値を受けとって同期処理を行いたいですよね。  
