...
styleタグは以下のようにheadタグ内にで書くことができます。
```html
<!DOCTYPE html>
<html>
  <head>
...
カラーコードは # の後ろに6文字の英数字で形式で書くことができます。
...
marginプロパティの内側の余白、paddingプロパティは外側の余白を設定することができます。  
大きさの指定方法はfont-sizeプロパティと同じく、 数値+単位(px, %, pt, em) です。
{{< tabs groupId="web_4_6" >}}
{{% tab name="index.html" %}}
```html
...
また、borderプロパティのみ場合でテーブルをレイアウトを設定すると線と線の間に隙間ができてしまいます。  
